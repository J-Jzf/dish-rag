...
from dish_rag.agent.nodes import AgentNodes, route_after_action, route_after_judge, route_after_retrieve
...
def build_graph(nodes: AgentNodes, checkpoint_path: Path):
    ...
    graph.add_node("prepare_action", nodes.prepare_action)
    ...
    graph.add_node("capture_action_result", nodes.capture_action_result)
    ...
    graph.add_edge("prepare_action", "rewrite_query")
    ...
    graph.add_edge("update_cooking_state", "capture_action_result")
    graph.add_conditional_edges(
